...
export const FounderSection: React.FC<FounderSectionProps> = ({ isLightMode = true }) => {
  return (
    <section id="founder" className={`py-20 transition-colors duration-200 relative border-t ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-5 relative">
            <div className={`relative rounded-3xl p-8 border shadow-2xl overflow-hidden transition-colors ${
              isLightMode
              ...
              <div className={`relative aspect-square rounded-2xl border flex flex-col items-center justify-center p-6 text-center shadow-xs ${
                isLightMode
                  ? 'bg-slate-50 border-slate-200'
                  : 'bg-gradient-to-br from-zinc-800 to-zinc-950 border-orange-500/20'
              }`}>
                <div className="w-24 h-24 rounded-2xl bg-orange-500/10 border-2 border-orange-500 flex items-center justify-center mb-4 shadow-lg shadow-orange-500/20">
                  <span className="font-black text-3xl text-orange-500">SA</span>
                </div>
                <h3 className={`text-2xl font-black ${isLightMode ? 'text-slate-900' : 'text-white'}`}>{COMPANY_INFO.founder.name}</h3>
              ...
              <div className="grid grid-cols-2 gap-2 mt-4 text-xs font-bold">
                <a
                  ...
                  className="p-3 rounded-xl bg-orange-600 hover:bg-orange-500 text-white flex items-center justify-center gap-2 transition-colors shadow-xs"
                >
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="space-y-4 pt-2">
              <div className={`p-4 rounded-2xl border flex items-start gap-3 ${
                ...
                <div className="p-2 rounded-xl bg-orange-500/10 border border-orange-500/30 text-orange-600 shrink-0">
                  <Terminal className="w-5 h-5" />
                </div>
                <div>
                  <h4 className={`text-sm font-bold ${isLightMode ? 'text-slate-900' : 'text-white'}`}>Full-Stack Technical Craftsmanship</h4>
                  <p className={`text-xs mt-1 ${isLightMode ? 'text-slate-600' : 'text-zinc-400'}`}>
              ...
              <div className={`p-4 rounded-2xl border flex items-start gap-3 ${
                ...
                <div>
                  <h4 className={`text-sm font-bold ${isLightMode ? 'text-slate-900' : 'text-white'}`}>Client-Centric Product Execution</h4>
                  <p className={`text-xs mt-1 ${isLightMode ? 'text-slate-600' : 'text-zinc-400'}`}>
